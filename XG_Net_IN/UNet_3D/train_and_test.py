...
from losses import myLoss, FocalLoss, DSC_LOSS


os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = "1"
import sys
sys.path.append(r'/data/zhangnaiwen442/testNorm/fold_2/XG_Net_IN/UNet_3D')
...
def DSC_2(pred, Y):
    ...
    return 1-(2*pred_gt_sum)/(pred_sum+gt_sum)
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.SGD(model.parameters(), 1e-5, momentum=0.3, weight_decay=0.0, nesterov=False)
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                for valnum, TestData in enumerate(test_dataloader):
                    ...
                    for vallayer in range(0, int(test_image.shape[2]/16)):
                        validpred = model(test_image[:, :, vallayer*16:(vallayer+1)*16, :, :])
                        ...
                        vxpred[vallayer*16:(vallayer+1)*16, :, :] += np.squeeze(validpredmax)
                    vxpred[vxpred > 0.5] = 1
                    Dice = DSC(vxpred, test_label)
                    ...
                    save_path = '/data/zhangnaiwen442/testNorm/fold_2/predImage3d/' + str(epoch) + '_1214_' + str(valnum) + '.nii.gz'
