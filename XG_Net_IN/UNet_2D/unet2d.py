...
class UNet2D(nn.Module):
    ...
    def __init__(self, n_class=1, act='lrelu'):
        ...
        self.block1 = DownTransition(1, 64, act)
        self.block2 = DownTransition(64, 128, act)
        self.block3 = DownTransition(128, 256, act)
        self.block4 = DownTransition(256, 512, act)

        self.convmid = nn.Sequential(LUConv(512, 1024,3,act),
                                     LUConv(1024, 512,3,act))

        self.block5 = UpTransition(512, 256,act)
        self.block6 = UpTransition(256,128,act)
        self.block7 = UpTransition(128,64,act)
        self.block8 = UpTrans(64,32,act)

        self.block9 = OutputTransition(32, n_class)


    def forward(self, x):
        self.b1, self.skip_b1 = self.block1(x)
        print(self.b1.shape, self.skip_b1.shape)

        self.b2,self.skip_b2 = self.block2(self.b1)
        print(self.b2.shape, self.skip_b2.shape)

        self.b3,self.skip_b3 = self.block3(self.b2)
        print(self.b3.shape, self.skip_b3.shape)

        self.b4,self.skip_b4 = self.block4(self.b3)
        print(self.b4.shape, self.skip_b4.shape)
